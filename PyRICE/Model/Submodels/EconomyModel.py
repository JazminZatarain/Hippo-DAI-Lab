...
class EconomyModel:
    ...
    def __init__(self, data_sets, steps, scenario_cback, regions_list):
        ...
        self.Abetement_cost = np.zeros((self.n_regions, steps))
        self.Abetement_cost_RATIO = np.zeros((self.n_regions, steps))
        self.Mabetement_cost = np.zeros((self.n_regions, steps))
    ...
    def init_economic_parameters(self, data_sets, damage_function, damage_parameters, temp_atm, dam_frac_global, miu):
        ...
        self.Y_gross[:, 0] = (
                    self.tfp_region[:, 0] * ((self.region_pop[:, 0] / 1000) ** (1 - self.gama)) * (
    ...
    def init_net_economy(self, data_sets, miu, S, elasticity_of_damages):
        ...
        self.Abetement_cost_RATIO[:, 0] = self.cost1[:, 0] * (miu[:, 0] ** self.expcost2)

        # Abettement cost total
        self.Abetement_cost[:, 0] = self.Y_gross[:, 0] * self.Abetement_cost_RATIO[:, 0]

        # Marginal abetement cost
        self.Mabetement_cost[:, 0] = self.pbacktime[:, 0] * miu[:, 0] ** (self.expcost2 - 1)

        # Resulting carbon_model price
        self.CPRICE[:, 0] = self.pbacktime[:, 0] * 1000 * (miu[:, 0] ** (self.expcost2 - 1))
    ...
    def run_gross_economy(self, data_sets, scenario_pop_gdp, tstep, t, longrun_scenario, long_run_nordhaus_pop_gr,
        ...
        if scenario_pop_gdp != 0:
            ...
            self.tfp_region[:, t] = self.Y_gross[:, t] / (
                    (self.k_region[:, t] ** self.gama) * (self.region_pop[:, t] / 1000) ** ( 1 - self.gama))
        ...
        if longrun_scenario == 1:
            ...
            # determine capital stock at time t
            self.k_region[:, t] = self.k_region[:, t - 1] * (
                        (1 - self.dk) ** tstep) + tstep * self.I[:, t - 1]
    ...
    def run_net_economy(self, data_sets, t, year, damage_function, damage_parameters, temp_atm, SLRDAMAGES,
        ...
        if year > self.backstopcompetitiveyear:
            ...
            self.pbacktime[:, t] = 0.10 * self.cback_region + (
                    self.pbacktime[:, t - 1] - 0.1 * self.cback_region) * (1 - self.decl_back_gr)

        # Adjusted cost for backstop
        self.cost1[:, t] = ((self.pbacktime[:, t] * self.sigma_region[:, t]) / self.expcost2)

        # Abettement cost ratio of output
        self.Abetement_cost_RATIO[:, t] = self.cost1[:, t] * (miu[:, t] ** self.expcost2)
        self.Abetement_cost[:, t] = self.Y_gross[:, t] * self.Abetement_cost_RATIO[:, t]

        # Marginal abetement cost
        self.Mabetement_cost[:, t] = self.pbacktime[:, t] * (miu[:, t] ** (self.expcost2 - 1))

        # Resulting carbon_model price
        self.CPRICE[:, t] = self.pbacktime[:, t] * 1000 * (miu[:, t] ** (self.expcost2 - 1))
        ...
        self.CPC_post_damage[year] = np.where(self.CPC_post_damage[year] > self.CPC_lo,
