...
This is a stochastic simulation model implementation of the RICE2010 model by Nordhaus. It is modified in way, such that alternative ethical problem formulations can be selected. Concretely, next to the standard `utilitarian` problem formulation, the user can alternatively choose a `prioritarian`, `sufficientarian`, or an `egalitarian` problem formulation.
...
4. [The PyRICE Model](#4-the-pyrice-model)
   ...
   2. [XLRM](#42-xlrm)
   3. [Initialization](#43-intialize-the-pyrice-model)
   4. [Run](#44-run-the-pyrice-model)
   5. [Results](#45-view-results)
...
│   │   ├── ClimatModel.py          
│   │   ├── EconomyModel.py         
│   │   └── UtilityModel.py         # Contains different welfare functions (utilitarian, prioritarian, sufficientarian, and egalitarian)
│   ├── DataSets.py                 
...
├── Main.ipynb                      # Example notebook that shows how to run the PyRICE model and view the results
└── README.md          
```
...
### 4.3 Intialize the PyRICE Model
...
With `model.get_better_formatted_results()`, you will get this object.
