...
def create_a_reference_set(problem, n_decision_vars, n_objs, problem_formulation='WEITZMAN/SUFFICIENTARIAN', id=198):
    """
    ...
    @return:
        ref_set: list
    """
    data = pd.read_csv(directory_optimization + f'/results/hypervolume/{problem_formulation}/archive_{id}.csv',
                       index_col=0).iloc[:, :n_decision_vars+n_objs]
...
def load_and_merge_archives(problem, n_decision_vars, n_objs, problem_formulation='WEITZMAN/SUFFICIENTARIAN', id=198):
    ...
    for i in range(2, id+1):
        archive = pd.read_csv(f'./results/hypervolume/{problem_formulation}/archive_{i}.csv')
...
def wrapper_hypervolume(problem_formulation='WEITZMAN/SUFFICIENTARIAN', id=198):
    ...
    ref_set = create_a_reference_set(problem, n_decision_vars, n_objs, problem_formulation, id)
    archives = load_and_merge_archives(problem, n_decision_vars, n_objs, problem_formulation, id)
...
def plot_hypervolumes(hypervolume_dict, saving=False):
    ...
    fig, axes = plt.subplots(nrows=1, ncols=4, sharex='all', figsize=(20, 4), tight_layout=True)
